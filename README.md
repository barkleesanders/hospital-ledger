...
| Hospitals with a standardized on-site price preview | **3,767** | `public/data/prices/index.json` |
...
| Payer-negotiated rate cells | **166.3 M** | sum of `n_rates` in `data/_payer_raw.jsonl` |
| Hospitals with at least one payer-rate row | **3,117** | distinct CCN in `_payer_raw.jsonl` |
| CCN × raw-payer-string rows | **76,242** | line count of `_payer_raw.jsonl` |
...
| 4 | Fetch + parse alive MRFs (CSV-tall/wide, JSON v2/v3, XLSX) | live (3,767 / 3,986 = 94.5% of required+live hospitals parsed; 163 terminal exceptions) |
...
> 4)." That is now false: Stage 4 is live, and 3,767 hospitals' MRFs have been
