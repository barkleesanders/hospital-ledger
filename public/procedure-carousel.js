...
(() => {
	...
	function init(root) {
		...
		for (let i = 0; i < dots.length; i++) {
			((idx) => {
				dots[idx].addEventListener("click", (e) => {
					e.preventDefault();
					pauseAutoplayUntilInteractionEnds();
					scrollTo(idx);
				});
		...
		function pauseAutoplayUntilInteractionEnds() {
			...
			if (pauseAutoplayUntilInteractionEnds._t)
				clearTimeout(pauseAutoplayUntilInteractionEnds._t);
			pauseAutoplayUntilInteractionEnds._t = setTimeout(() => {
