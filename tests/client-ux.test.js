...
function testProcedureNameSearch() {
	...
	console.log(
		`Match: ${matches[0] ? `${matches[0][0]} - ${matches[0][1]}` : "none"}`,
	);
...
function testMultipleMatches() {
	...
	console.log(`Search term: "${searchTerm}"`);
	console.log(`Matches found: ${matches.length}`);
	for (const [code, desc] of matches) {
...
function testCaseInsensitive() {
	...
	for (const [i, term] of terms.entries()) {
		console.log(`  ${term}: ${results[i]} matches`);
	}
...
function testFilterStaging() {
	...
	console.log(pass ? "✓ PASS" : "✗ FAIL");
	console.log(
		"Note: Without client script, filters don't work. This tests the staging concept.",
	);
...
function testUrlParamSupport() {
	...
	const url = new URL(
		"http://localhost:8787/procedure/45378?state=CA&payer=aetna&sort=cash-asc",
	);
...
function testSortFunctions() {
	...
	for (const h of byState) {
		console.log(`  ${h.name} (${h.state})`);
	}

	// Test cash ascending
	const byCash = [...hospitals].sort((a, b) => a.cash - b.cash);
	console.log("Sort by cash (ascending):");
	for (const h of byCash) {
		console.log(`  ${h.name} - $${h.cash}`);
	}
