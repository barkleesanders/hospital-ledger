...
import { BUNDLED_SUMMARY, loadSummary, type Summary } from "../lib/site-data";
...
const PUBLIC_PAGES: PublicPage[] = [
	{
		...
		path: "/",
		render: (summary) => String(homePage(`${SITE_ORIGIN}/`, summary)),
	},
	{
		title: "About the numbers (methodology)",
		path: "/about-the-numbers",
		render: (summary) =>
			String(aboutPage(`${SITE_ORIGIN}/about-the-numbers`, summary)),
	},
	{
		title:
...
export function buildSiteCorpus(
	summary: Summary = BUNDLED_SUMMARY,
): FaqCorpusDoc[] {
	return [
		...PUBLIC_PAGES.map((p) => ({
			title: p.title,
			url: `${SITE_ORIGIN}${p.path}`,
			text: pageText(p.render(summary)),
		})),
		gradesDoc(),
	];
}

let siteCached: { generatedAt: string; docs: FaqCorpusDoc[] } | null = null;

/**
 * Corpus loader for mountInfiniteFaq: the site's own pages, rendered once per isolate
 * per data release. The counts come from R2 meta/summary.json (src/lib/site-data.ts),
 * the same object the pages render from, so the assistant never quotes a number the
 * page beside it has already replaced; the corpus is rebuilt only when generated_at
 * moves. Without env (tests) it renders the bundled copy.
 */
export async function loadFaqCorpus(env?: Bindings): Promise<FaqCorpusDoc[]> {
	const summary = env ? (await loadSummary(env)).summary : BUNDLED_SUMMARY;
	if (!siteCached || siteCached.generatedAt !== summary.generated_at) {
		siteCached = {
			generatedAt: summary.generated_at,
			docs: buildSiteCorpus(summary),
		};
	}

	return siteCached.docs;
}
