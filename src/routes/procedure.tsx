	...
	return (
		...
		>
			...
			<main class="mx-auto max-w-6xl px-4 sm:px-6 py-8">
				<h1 class="serif text-4xl sm:text-5xl md:text-6xl text-zinc-50">
					{fallbackName}
				</h1>
				<div class="label-eyebrow mt-3">
					<span class="mono text-zinc-300">{escapeHtml(code)}</span> · not in
					our top-5,000 index
				</div>
				<div class="mt-8 rounded-lg border border-amber-700/40 bg-amber-900/15 p-4 text-amber-100 max-w-2xl">
					<div class="font-semibold mb-1">
						No price data for code <span class="mono">{escapeHtml(code)}</span>
					</div>
					<div class="text-sm text-amber-200/90">
						We index the top 5,000 most-coverage CPT/HCPCS codes. This code may
						be:
						<ul class="list-disc ml-5 mt-1 space-y-0.5">
							<li>A rare or specialty procedure not in our top 5k</li>
							<li>
								A typo (CPT codes are 5 digits; HCPCS are letter + 4 digits)
							</li>
						...
						<div class="mt-2">
							<a
								href="/"
								class="inline-block rounded-md bg-amber-700/30 hover:bg-amber-700/50 px-3 py-1 text-amber-50"
							>
...
function ledeMarkup(data: ProcedureData) {
	...
	return (
		<>
			...
			<span class="text-amber-300 tab-num">{fmtMoney(cash_min)}</span> at one
			hospital and{" "}
			<span class="text-rose-300 tab-num">{fmtMoney(cash_max)}</span> at another
			— a <strong class="text-zinc-100">{spread}× spread</strong> for what's
			billed under the same code. Median:{" "}
			<span class="text-emerald-300 tab-num">{fmtMoney(cash_p50)}</span>.
		</>
	);
}
...
function hospitalCards(data: ProcedureData) {
	...
	if (!rows.length) {
		return (
			<div class="px-3 py-6 text-center text-zinc-500 text-sm">No matches.</div>
		);
	}
		...
		return (
			...
			>
				<div class="flex items-baseline justify-between gap-2 mb-2 min-w-0">
					<div class="font-medium text-emerald-300 truncate text-sm">
						{h.name}
					</div>
				...
				<div class="grid grid-cols-2 gap-x-3 gap-y-1 text-xs">
					...
					<div class="text-right tab-num text-zinc-400">
						{fmtMoney(h.gross)}
					</div>
				</div>
			</a>
...
function hospitalRows(data: ProcedureData) {
	...
	return sorted.map((h) => {
		...
		const isHigh = h.quality === "high_outlier";
		const cashClass =
			isLow || isHigh ? "text-amber-200" : "text-emerald-300 font-medium";
		const flagBadge = isLow ? (
			<span class="ml-1 inline-flex items-center px-1.5 py-0.5 rounded text-[10px] bg-amber-900/30 text-amber-300 border border-amber-700/40">
				⚠ check
			</span>
		...
		return (
			<tr
				class={`hover:bg-zinc-900/50 ${isLow || isHigh ? "opacity-70" : ""}`}
				data-payers={JSON.stringify(h.payers ?? [])}
			>
				<td class="px-3 py-2">
					<a
				...
				<td class="hidden md:table-cell px-3 py-2 text-right tab-num text-zinc-300">
					{fmtMoney(h.gross)}
				</td>
				...
				<td class="hidden md:table-cell px-3 py-2 text-right tab-num text-zinc-400">
					{fmtMoney(h.min)}
				</td>
				<td class="hidden md:table-cell px-3 py-2 text-right tab-num text-zinc-400">
					{fmtMoney(h.max)}
			...
			<main class="mx-auto max-w-6xl px-4 sm:px-6 py-8">
				...
				<section class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 md:mb-12">
					...
					<div>
						<div class="label-eyebrow">
							Median across{" "}
							<span class="tab-num">
								{data.stats.hospital_count.toLocaleString("en-US")}
							</span>{" "}
							hospitals
						</div>
						<div class="mt-2 serif text-3xl sm:text-4xl md:text-5xl text-emerald-300">
							{fmtMoney(data.stats.cash_p50)}
						</div>
						<div class="mt-2 text-sm text-zinc-400 leading-snug">
							The realistic middle of the cash-price distribution. Use this as
							your benchmark.
						</div>
					</div>
					<div>
						<div class="label-eyebrow">Most expensive</div>
						<div class="mt-2 serif text-3xl sm:text-4xl md:text-5xl text-rose-300">
							{fmtMoney(data.stats.cash_max)}
						</div>
						<div class="mt-2 text-sm text-zinc-400 leading-snug">
							Highest published price. Often a gross-charge "list price" rarely
							actually paid.
						</div>
					</div>
				</section>
				...
				<section class="mb-8 md:mb-12">
					<div class="flex items-baseline justify-between mb-4 gap-2">
						<h2 class="serif text-2xl sm:text-3xl text-zinc-100">
							Price distribution
						</h2>
						<div class="label-eyebrow">
							{data.hospitals
					...
					<div class="rounded-md border border-zinc-800 bg-zinc-900/30 p-4">
						<PriceHistogram
							...
							cashMedian={data.stats.cash_p50}
						/>
					</div>
					<p class="mt-3 text-xs text-zinc-500 leading-relaxed">
						Each bar = number of hospitals charging in that price range.{" "}
						<span class="text-emerald-300">Emerald</span> bar contains the
						median. <span class="text-amber-300">Amber</span> dots = entries we
						flagged as likely partial-cost line items (excluded from the table
						below).
					</p>
				</section>
				...
				<section
					...
					data-payers={JSON.stringify(
						[...payers.entries()].sort((a, b) => a[1].localeCompare(b[1])),
					)}
					...
					<div class="grid grid-cols-1 md:grid-cols-3 gap-3">
						...
						<div>
							...
							<select
								...
								class="mt-1 w-full rounded-md border border-zinc-700 bg-zinc-950 px-3 py-2 text-sm"
							>
								<option value="median-first">
									Most representative (near median)
								</option>
								...
								<option value="payer-asc">
									Lowest your-insurer rate first
								</option>
							</select>
						</div>
					</div>
				...
				<p class="mt-6 text-xs text-zinc-500">
					Data straight from each hospital's federally-mandated machine-readable
					file (45 CFR § 180). Prices reflect what the hospital published; what
					you actually pay depends on your specific plan, deductible, and other
					factors. "Cash price" is the discounted self-pay rate hospitals are
					required to publish for uninsured patients.
				</p>
			</main>
		</Layout>
	);
