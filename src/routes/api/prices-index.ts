...
async function fromR2(env: Env["Bindings"]): Promise<IndexCandidate | null> {
  ...
  return {
    ...
    contentType: obj.httpMetadata?.contentType ?? "application/json; charset=utf-8",
  };
}

async function fromAssets(env: Env["Bindings"], request: Request): Promise<IndexCandidate | null> {
...
export async function pricesIndexHandler(c: Context<Env>): Promise<Response> {
  ...
  const [r2, assets] = await Promise.all([fromR2(env), fromAssets(env, request)]);
  const candidates = [r2, assets].filter((x): x is IndexCandidate => x !== null);
  if (!candidates.length) return json({ error: "price index not found" }, { status: 404 });
  candidates.sort((a, b) => b.count - a.count);
  return indexResponse(candidates[0]!);
}
