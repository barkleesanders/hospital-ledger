...
function displayCodeAndType(item: RawItem): { code: string; codeType: string } {
	...
	let codeType =
		String(item?.code_type ?? "")
			.trim()
			.toUpperCase() || "CDM";
	const payerRates = Array.isArray(item?.payer_rates)
		? (item.payer_rates as PayerRate[])
		: [];
	...
	if (
		!code &&
		desc &&
		(gross !== null || cash !== null || payerRates.length > 0)
	) {
...
function slimParsedRecord(parsed: unknown, ccn: string): SlimRecord | null {
	...
	const items: RawItem[] = Array.isArray(p?.items)
		? (p.items as RawItem[])
		: [];
	...
	for (const item of items) {
		...
		const prevPayerLen =
			prev && Array.isArray(prev.payer_rates)
				? (prev.payer_rates as unknown[]).length
				: 0;
	...
	for (const item of seen.values()) {
		...
		const payers: SlimPayer[] = payerRates
			...
			.sort(
				(a, b) =>
					(numeric(b?.rate_dollar) ?? 0) - (numeric(a?.rate_dollar) ?? 0),
			)
			.slice(0, 5)
			.map((payer) => ({
...
async function fetchPreviewFromParsedR2(
	env: Env["Bindings"],
	ccn: string,
): Promise<Response | null> {
	if (!env.HL_MRF_PARSED || typeof env.HL_MRF_PARSED.get !== "function")
