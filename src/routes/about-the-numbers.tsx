	...
	return (
		...
		>
			<main class="mx-auto max-w-3xl px-4 sm:px-6 py-10 sm:py-14">
				<p class="text-xs uppercase tracking-widest text-emerald-400">
					Methodology
				</p>
				...
				<p class="mt-4 text-zinc-400 text-sm">
					Updated <span class="tab-num">{updated}</span> · all figures derived
					from{" "}
						...
						<table class="w-full text-sm">
							...
							<tbody class="text-zinc-300">
								<tr class="border-b border-zinc-900">
									...
									<td class="py-3 pr-3 text-right tab-num">
										{fmt(totalFacilities)}
									</td>
									<td class="py-3 text-zinc-400">
										Rows in CMS Hospital General Information.
									</td>
								</tr>
								<tr class="border-b border-zinc-900">
									...
									<td class="py-3 text-zinc-400">
										Acute care, critical access, children's, and rural emergency
										hospitals — the set 45 CFR § 180 binds.
									</td>
								</tr>
								<tr class="border-b border-zinc-900">
									...
									<td class="py-3 text-zinc-400">
										CMS-required hospitals where we verified a live,
										downloadable MRF URL.
									</td>
								</tr>
								<tr>
									<td class="py-3 pr-3 font-medium text-emerald-300">
										Standardized prices
									</td>
									<td class="py-3 pr-3 text-right tab-num text-emerald-300">
										{fmt(standardized)}
									</td>
									<td class="py-3 text-zinc-400">
										Live MRFs we successfully parsed into{" "}
										<code class="mono text-zinc-300">n &gt; 0</code>{" "}
										standardized rows. This is the homepage headline.
									</td>
								</tr>
							</tbody>
						</table>
				...
				<section class="mt-12">
					...
					<p class="mt-3 text-zinc-300 leading-relaxed">
						Between "live MRF" ({fmt(compliant)}) and "standardized prices" (
						{fmt(standardized)}) sits a coverage hole of{" "}
						<span class="tab-num font-semibold text-white">
							{fmt(totalGap)}
						</span>{" "}
						hospitals. Two failure modes:
					</p>
					<ul class="mt-4 space-y-3 text-zinc-300 leading-relaxed">
						<li class="flex gap-3">
							...
							<span>
								...
								hospitals parsed to <span class="tab-num">0</span> rows. Their
								MRF is downloadable but the file is empty, placeholder, or in a
								format the parser produced no usable rows from (e.g. an XML/PDF
								that didn't yield CDM / CPT / DRG / HCPCS rows). These show up
								in{" "}
								<a
									href="/data/prices/index.json"
									class="underline hover:text-zinc-200"
								>
						...
						<li class="flex gap-3">
							...
							<span>
								...
								hospitals aren't in the price index at all yet. They have a live
								MRF (we can fetch the URL) but ingest has either failed
								(decompression, schema, encoding) or hasn't been attempted in
								the current build. These are queued for the next ingest pass.
							</span>
						</li>
					</ul>
					<p class="mt-4 text-sm text-zinc-400 leading-relaxed">
						We surface only <code class="mono text-zinc-300">n &gt; 0</code>{" "}
						entries on the homepage because zero-row outputs aren't useful for
						comparison. They count toward the parse-pipeline yield, not toward
						the patient- facing count.
					</p>
				</section>
				...
				<section class="mt-12">
					...
					<ol class="mt-4 space-y-3 list-decimal list-inside text-zinc-300 leading-relaxed">
						<li>
							<span class="font-medium">Seed</span> — start with CMS hospital
							list (<span class="tab-num">{fmt(totalFacilities)}</span>) and the
							CMS HPT seed.
						</li>
						<li>
							<span class="font-medium">Discover</span> — sitemap walker +
							email-domain expansion + Wayback fallback to locate each
							hospital's MRF URL.
						</li>
						<li>
							<span class="font-medium">Probe</span> — HEAD / partial GET to
							confirm the URL is alive and serves a non-HTML, non-XML, non-PDF
							body. <span class="tab-num">{fmt(compliant)}</span> of{" "}
							<span class="tab-num">{fmt(cmsRequired)}</span> CMS-required
							hospitals pass this gate.
						</li>
						<li>
							<span class="font-medium">Parse → standardize</span> — extract CDM
							/ CPT / DRG / HCPCS rows into a uniform schema.{" "}
							<span class="tab-num">{fmt(indexEntries)}</span> hospitals have an
							index entry;{" "}
							<span class="tab-num font-semibold text-emerald-300">
								{fmt(standardized)}
							</span>{" "}
							of those have at least one usable row.
						</li>
						<li>
							<span class="font-medium">Publish</span> — write per-hospital JSON
							to R2 +{" "}
							<a
								href="/data/prices/index.json"
								class="underline hover:text-zinc-200"
				...
				<section class="mt-12">
					...
					<p class="mt-3 text-zinc-300 leading-relaxed">
						The closeout pipeline (
						<code class="mono text-zinc-300">scripts/coverage_closeout.py</code>
						) replays the ingest pass against the{" "}
						<span class="tab-num">{fmt(notInIndex)}</span> not-in-index CCNs and
						the <span class="tab-num">{fmt(zeroEntries)}</span> zero-row CCNs
						with auto-tuned parallelism. Two open issues:
					</p>
					<ul class="mt-4 space-y-2 text-zinc-300 leading-relaxed text-sm">
						<li>
							<span class="text-emerald-400 mr-2">▸</span> A few host clusters
							(e.g. <code class="mono">apps.para-hcfs.com</code>,{" "}
							<code class="mono">sthpiprd.blob.core.windows.net</code>) account
							for most of the parse failures — format quirks rather than
							transient errors.
						</li>
						<li>
							<span class="text-emerald-400 mr-2">▸</span> Zero-row outputs are
							mostly placeholder files with no price content (CMS technically
							counts them as "available" even though they teach you nothing).
							The law's hole, not ours.
						</li>
					</ul>
				</section>
