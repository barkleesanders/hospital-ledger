...
function notFoundPage(slug: string, url: string) {
	return (
		<Layout
			title={`Insurance ${slug} — Hospital Ledger`}
			description={`No data for insurance ${slug} yet.`}
			url={url}
		>
...
function payerPage(slug: string, data: PayerData, url: string) {
	...
	const states = [
		...new Set(data.hospitals.map((h) => h.state).filter(Boolean)),
	].sort();
		...
		>
			<header class="border-b border-zinc-800 bg-gradient-to-b from-zinc-900 to-zinc-950">
				<div class="mx-auto max-w-6xl px-4 sm:px-6 py-6 sm:py-8">
					...
					<p class="mt-2 text-zinc-400">
						{hospitalCount.toLocaleString("en-US")} hospitals have negotiated
						rates with this insurance.
					</p>
				</div>
			</header>
			...
			<main class="mx-auto max-w-6xl px-4 sm:px-6 py-8">
				<section class="grid grid-cols-2 md:grid-cols-3 gap-3 mb-6">
					...
					<div class="rounded-lg border border-zinc-800 bg-zinc-900/60 p-4">
						...
						<div class="mt-1 text-2xl font-semibold tab-num text-zinc-100">
							{fmtMoney(p.median_rate)}
						</div>
					</div>
					<div class="rounded-lg border border-zinc-800 bg-zinc-900/60 p-4">
						<div class="text-xs uppercase tracking-wider text-zinc-500">
							Plan name aliases on MRFs
						</div>
						...
						<div>
							...
							>
								<option value="items-desc">
									Most procedures negotiated first
								</option>
							...
							>
								...
								<div class="grid grid-cols-2 gap-x-3 gap-y-1 text-xs">
									...
									<div class="text-right tab-num text-base font-medium text-emerald-300">
										{fmtMoney(h.median_rate)}
									</div>
									<div class="text-zinc-500">Procedures w/ rate</div>
									<div class="text-right tab-num text-zinc-300">
							...
							<tbody id="results" class="divide-y divide-zinc-800">
								{rows.map((h) => (
									<tr class="hover:bg-zinc-900/50">
										...
										<td class="px-3 py-2 text-right tab-num text-emerald-300 font-medium">
											{fmtMoney(h.median_rate)}
										</td>
				...
				<p class="mt-6 text-xs text-zinc-500">
					"Negotiated rate" is what each hospital agreed to accept from this
					insurance for a specific procedure. Your actual cost depends on your
					plan's deductible, copay, coinsurance, and out-of-network rules. Some
					plans hospitals call "Aetna PPO" or "Aetna HMO" lump under{" "}
					<em>Aetna</em> here — see plan-name aliases above.
				</p>
			</main>
		</Layout>
	);
