...
export const PriceHistogram: FC<PriceHistogramProps> = ({
	hospitals,
	cashMedian,
}) => {
	...
	const medBin = Math.min(
		NBINS - 1,
		Math.floor((Math.log10(cashMedian) - logLo) / step),
	);

	const xFor = (i: number): number => PADDING.l + (i / NBINS) * innerW;
	const xWidth = innerW / NBINS - 2;
	const yFor = (c: number): number =>
		PADDING.t + innerH - (c / maxCount) * innerH;

	const lowOutliers = hospitals.filter(
		(h) => h.quality === "low_outlier" && h.cash !== null,
	) as Array<HospitalPoint & { cash: number }>;
	const highOutliers = hospitals.filter(
		(h) => h.quality === "high_outlier" && h.cash !== null,
	) as Array<HospitalPoint & { cash: number }>;
	const allValues = [
		...cashVals,
		...lowOutliers.map((h) => h.cash),
		...highOutliers.map((h) => h.cash),
	];
	...
	const xForVal = (v: number): number => {
		...
		return (
			PADDING.l +
			((Math.log10(Math.max(1, v)) - fullLogLo) / fullRange) * innerW
		);
	};
	...
	return (
		<svg
			...
			role="img"
			aria-label="Cash price distribution histogram"
		>
			<title>Cash price distribution</title>
			<line
				x1={PADDING.l}
				y1={axisY}
				x2={W - PADDING.r}
