...
export async function loadProcedure(
	...
	const r2 = await readR2Json<ProcedureData>(
		env.HL_MRF_PARSED,
		`aggregates/cpt-detail/${upper}.json`,
	);
	if (r2) return r2;
	return await fetchAssetJson<ProcedureData>(
		env,
		request,
		`/data/cpt-detail/${upper}.json`,
	);
}
...
export async function loadPayer(
	...
	request: Request,
	slug: string,
): Promise<PayerData | null> {
	const lower = slug.toLowerCase();
	const r2 = await readR2Json<PayerData>(
		env.HL_MRF_PARSED,
		`aggregates/payer/${lower}.json`,
	);
	if (r2) return r2;
	return await fetchAssetJson<PayerData>(
		env,
		request,
		`/data/payer/${lower}.json`,
	);
}
...
export async function loadHospital(
	...
	const r2 = await readR2Json<HospitalData>(
		env.HL_MRF_PARSED,
		`prices/${ccn}.json`,
	);
	if (r2) return r2;
	return await fetchAssetJson<HospitalData>(
		env,
		request,
		`/data/prices/${ccn}.json`,
	);
}
