...
export function json<T>(body: T, init: JsonInit = {}): Response {
  ...
  const cacheControl = init.cacheControl ?? (status === 200 ? DEFAULT_CACHE : NO_STORE);
...
export function htmlResponse(html: string, init: { status?: number; cacheControl?: string } = {}): Response {
  const status = init.status ?? 200;
