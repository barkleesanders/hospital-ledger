...
async function fromR2(env) {
  ...
  const obj = await env.HL_MRF_PARSED.get('aggregates/payers-index.json');
  if (!obj) return null;
...
export async function onRequestGet({ env, request }) {
  return (await fromR2(env)) || (await fromAssets(env, request)) || json({ error: 'payers index not built yet' }, 404);
}
